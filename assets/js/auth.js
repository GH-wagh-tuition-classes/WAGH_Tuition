const WTC_AUTH = (() => {
  ...
  function redirectByRole(user) { const role = String(user.role || 'Student').toLowerCase(); if (role === 'teacher') location.href = 'teacher.html'; else if (role === 'admin') location.href = 'admin.html'; else if (role === 'parent') location.href = 'parent.html'; else location.href = 'student.html'; }
  function requireRole(role) { const user = getUser(); if (!user || String(user.role || '').toLowerCase() !== role.toLowerCase()) { location.href = 'index.html#login'; return null; } return normalizeUser(user); }
